...
export class JsonClaudeManager {
  ...
  create(
    ...
    if (spec.kind === 'resume') {
      ...
      // Blank spawn: if a transcript already exists for this id (probed in
      // the worktree's resolved Claude home), resume it; otherwise mint it
      // with --session-id. Honors a custom CLAUDE_CONFIG_DIR so a custom-home
      // session isn't misdetected as fresh on restart.
      claudeSessionId = sessionId
      const existingSession = existsSync(
        transcriptPathFor(sessionId, worktreePath, this.opts.getClaudeConfigDir(worktreePath))
      )
      resumeOrSet = existingSession
        ? ['--resume', sessionId]
